...
pub fn casecmp(left: &str, right: &str) -> bool {
    ...
    loop {
        match (left.next(), right.next()) {
            ...
            (Some('\u{0049}'), Some('\u{0069}')) => continue,
            (Some('\u{0069}'), Some('\u{0049}')) => continue,
            ...
            (Some('\u{0130}'), Some('\u{0069}')) if matches!(right.next(), Some('\u{0307}')) => {
                continue
            }
            (Some('\u{0069}'), Some('\u{0130}')) if matches!(left.next(), Some('\u{0307}')) => {
                continue
            }
